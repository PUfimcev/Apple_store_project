...
watch(enableNavigation, (newValue) => {
    if (newValue) {
...
window.addEventListener("resize", () => {
    enableNavigation.value = window.innerWidth < 992;
    console.log(enableNavigation.value)
});
onMounted(() => {
    window.addEventListener("resize", () => {
        enableNavigation.value = window.innerWidth < 992;
...
<template>
    <li v-if="product"
        ...
        <swiper-container
            ...
            :space-between="0"
            :loop="true"
            :modules="enableNavigation.value ? [Navigation, Pagination] : [Pagination]"
            :navigation="true"
